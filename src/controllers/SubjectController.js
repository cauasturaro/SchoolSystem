...
class SubjectController {
    ...
    static createSubject = async (req, res) => {
        try {
            const subject = SubjectService.create(req.body);
    ...
    static findAllSubjects= async (req, res) => {
        try {
            const subject = SubjectService.findAll();
    ...
    static updateSubjectById = async (req, res) => {
        ...
        try {
            const found = await SubjectService.update({ name, teacherName, course, }, { where: { id } });

            if (found === 0) 
                return res.status(404).json({ error: "Subject wasn't found or no changes to make"});
